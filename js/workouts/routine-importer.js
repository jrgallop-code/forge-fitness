...
function renderSharedWorkoutReview(page) {
  ...
  stage.innerHTML = `<div class="routine-import-review">
    ...
    <div class="routine-import-days">
      ${plan.days.map((day, dayIndex) => `<section class="routine-import-day"><div class="routine-import-day-head"><span class="eyebrow">DAY ${dayIndex + 1}</span><h4>${escapeHtml(day.name)}</h4></div><div class="routine-import-exercises">${day.exercises.map(item => `<article class="routine-import-exercise"><div class="routine-import-match"><div><strong>${escapeHtml(getAllExercises().find(exercise => exercise.id === item.id)?.name || item.id)}</strong><small>${Number(item.sets) || 0} sets${item.reps ? ` · ${escapeHtml(item.reps)} reps` : ""}</small></div></div></article>`).join("")}</div></section>`).join("")}
    </div>
