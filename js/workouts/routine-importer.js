...
function renderShell() {
  return `<section class="routine-import-wizard" data-routine-import-wizard hidden>
    <div class="routine-import-topbar"><div><span class="eyebrow">IMPORT ROUTINE <span class="routine-import-beta">BETA</span></span><h3>Paste your routine</h3><p>Copy a routine from Reddit, Notes, a website, or a message. You will review everything before it is saved.</p></div><button class="secondary-btn" type="button" data-routine-import-close>Close</button></div>
...
function showPaste(page) {
  ...
  if (importState) {
    ...
    const source = page.querySelector("#routine-import-source");
    if (textarea) textarea.value = importState.rawText || "";
    if (source) source.value = importState.source || "";
  }
  updateCounter(page);
}
...
function buildReview(page) {
  ...
  importState = { name: suggestedName(parsed.days), source, rawText: text, days: parsed.days, skipped: parsed.skipped };
  renderReview(page);
}
...
function renderExercise(item, dayIndex, exerciseIndex) {
  ...
  return `<article class="routine-import-exercise ${uncertain ? "needs-review" : ""}"><div class="routine-import-match"><div><strong>${escapeHtml(item.name)}</strong><small>${uncertain ? "Check this match" : "Matched"}</small></div><select data-routine-match data-day="${dayIndex}" data-exercise="${exerciseIndex}" aria-label="Exercise match for ${escapeHtml(item.name)}">${all.map(exercise => `<option value="${exercise.id}" ${exercise.id === item.match.exerciseId ? "selected" : ""}>${escapeHtml(exercise.name)}</option>`).join("")}</select>${uncertain ? `<button type="button" data-routine-confirm data-day="${dayIndex}" data-exercise="${exerciseIndex}">Use suggestion</button>` : ""}</div><div class="routine-import-fields"><label>Sets<input type="number" min="1" max="20" value="${item.sets}" data-routine-sets data-day="${dayIndex}" data-exercise="${exerciseIndex}"></label><label>Reps<input type="text" maxlength="20" value="${escapeHtml(item.reps)}" data-routine-reps data-day="${dayIndex}" data-exercise="${exerciseIndex}"></label><div class="routine-import-row-actions"><button type="button" data-routine-up data-day="${dayIndex}" data-exercise="${exerciseIndex}" aria-label="Move exercise up">↑</button><button type="button" data-routine-down data-day="${dayIndex}" data-exercise="${exerciseIndex}" aria-label="Move exercise down">↓</button><button type="button" data-routine-remove data-day="${dayIndex}" data-exercise="${exerciseIndex}">Remove</button></div></div>${item.notes ? `<p class="routine-import-note">${escapeHtml(item.notes)}</p>` : ""}</article>`;
}
...
function saveRoutine(button, page) {
  ...
  const plan = { id: `import-${Date.now()}`, name, days: importState.days.map(day => ({ name: day.name, exercises: day.exercises.map(item => ({ id: item.match.exerciseId, sets: item.sets, reps: item.reps })) })), importedRoutine: { version: 1, sourceUrl: importState.source || null, originalText: importState.rawText, importedAt: new Date().toISOString() } };
