...
document.addEventListener("click", event => {
  ...
  if (button.matches("[data-smart-save]") && generated?.smartBuild?.version === 9) {
    event.preventDefault();
    event.stopImmediatePropagation();
    savePlan(button);
  }
...
function getSplitDefinition(days) {
  if (days === 2) return {
    ...
    mapping: Object.fromEntries(MUSCLES.map(m => [m, [0, 1]])),
    templates: [fullBodyTemplate(), fullBodyTemplate()]
  };
  if (days === 3) return {
    names: ["Full Body A", "Full Body B", "Full Body C"],
    mapping: Object.fromEntries(MUSCLES.map(m => [m, [0, 1, 2]])),
    templates: [fullBodyTemplate(), fullBodyTemplate(), fullBodyTemplate()]
  };
  if (days === 4) return {
    names: ["Upper A", "Lower A", "Upper B", "Lower B"],
    mapping: {
      Chest: [0, 2], Back: [0, 2], Shoulders: [0, 2], Biceps: [0, 2], Triceps: [0, 2],
      Quads: [1, 3], Hamstrings: [1, 3], Glutes: [1, 3], Calves: [1, 3], Core: [0, 1, 2, 3]
    },
    templates: [upperTemplate("Biceps"), lowerTemplate(), upperTemplate("Triceps"), lowerTemplate()]
  };
  if (days === 5) return {
    names: ["Upper", "Lower", "Push", "Pull", "Legs"],
    mapping: {
      Chest: [0, 2], Back: [0, 3], Shoulders: [0, 2, 3], Biceps: [0, 3], Triceps: [0, 2],
      Quads: [1, 4], Hamstrings: [1, 4], Glutes: [1, 4], Calves: [1, 4], Core: [0, 1, 2, 3, 4]
    },
    templates: [upperTemplate("Biceps"), lowerTemplate(), pushTemplate(), pullTemplate(), legsTemplate()]
  };
  return {
    names: ["Push A", "Pull A", "Legs A", "Push B", "Pull B", "Legs B"],
    mapping: {
      Chest: [0, 3], Back: [1, 4], Shoulders: [0, 1, 3, 4], Biceps: [1, 4], Triceps: [0, 3],
      Quads: [2, 5], Hamstrings: [2, 5], Glutes: [2, 5], Calves: [2, 5], Core: [0, 1, 2, 3, 4, 5]
    },
    templates: [pushTemplate(), pullTemplate(), legsTemplate(), pushTemplate(), pullTemplate(), legsTemplate()]
  };
}
...
function pushTemplate() {
  return [
    { muscle: "Chest", role: "press" },
    { muscle: "Chest", role: "chest-secondary" },
    { muscle: "Shoulders", role: "shoulder" },
    { muscle: "Triceps", role: "arm" }
  ];
}
function pullTemplate() {
  return [
    { muscle: "Back", role: "vertical-pull" },
    { muscle: "Back", role: "row" },
    { muscle: "Biceps", role: "arm" },
    { muscle: "Shoulders", role: "rear-delt" }
  ];
}
function legsTemplate() {
  return [
    { muscle: "Quads", role: "knee" },
    { muscle: "Hamstrings", role: "posterior" },
    { muscle: "Glutes", role: "glute" },
...
function generateProgram() {
  const split = getSplitDefinition(state.days);
  ...
  seedWorkoutStructure(days, split.templates, pool, usedCounts);
  ...
  const validation = validateProgram(days, targets, effective, exposureCounts, split, exerciseMap);
...
function ensurePriorityFrequency(days, mapping, pool, usedCounts) {
  for (const muscle of state.priorities) {
    ...
    for (const dayIndex of eligible) {
      ...
      if (addExercise(days[dayIndex], muscle, 2, pool, usedCounts, dayIndex, { role: muscle === "Core" ? "core" : "priority" })) current += 1;
    }
  }
}
...
function validateProgram(days, targets, effective, exposureCounts, split, exerciseMap) {
  const issues = [];
  days.forEach(day => {
  ...
  for (const muscle of MUSCLES) {
    if ((effective[muscle] || 0) > MAX_EFFECTIVE + 0.5 && muscle !== "Core") issues.push(`${muscle} exceeds the effective-volume ceiling`);
  }

  for (const muscle of state.priorities) {
    const eligible = split.mapping[muscle] || [];
    const desired = muscle === "Core" ? Math.min(2, eligible.length) : Math.min(2, eligible.length);
    if ((exposureCounts[muscle] || 0) < desired) issues.push(`${displayMuscle(muscle)} priority frequency is too low`);
    if ((effective[muscle] || 0) + 1 < Math.min(targets[muscle], muscle === "Core" ? 8 : 10)) issues.push(`${displayMuscle(muscle)} priority volume is too low`);
  }
...
function buildPlanName() {
  ...
  return `${state.days}-Day ${GOAL_LABELS[state.goal]} Program${priority}`;
}
...
function renderReview() {
  ...
  const status = valid
    ? `<div class="smart-volume-grid"><div class="priority"><span>Program validation</span><strong>Passed ✓</strong></div><div><span>Session structure</span><strong>4+ exercises protected</strong></div></div>`
  ...
  host.innerHTML = `<div class="smart-review"><span class="eyebrow">YOUR PROGRAM</span><h3>${escapeHtml(generated.name)}</h3><div class="smart-review-meta"><span>${state.days} days/week</span><span>~${state.duration} min/session</span><span>${capitalize(state.experience)}</span><span>${escapeHtml(GOAL_LABELS[state.goal])}</span></div><h4>Program guardrails</h4><p class="smart-helper">Smart Build establishes balanced workout structure first, then allocates weekly effective volume. Priority muscles get earlier placement, higher targets and added frequency where the split allows. Compound sets contribute 0.5 effective-set credit to commonly involved secondary muscles.</p>${status}<h4>Weekly muscle stimulus</h4><div class="smart-volume-grid effective">${volumeRows}</div><h4>Workout days</h4><div class="smart-review-days">${dayRows}</div><p class="smart-review-note">Volume targets are evidence-informed starting ranges rather than rigid prescriptions. Experience, training frequency, session time, muscle priority and indirect compound work all influence the final allocation.</p><div class="smart-review-actions"><button class="secondary-btn" type="button" data-smart-edit>Edit Answers</button><button class="secondary-btn" type="button" data-smart-regenerate-exercises>New Exercises</button><button class="secondary-btn" type="button" data-smart-regenerate-program>New Program</button><button class="primary-btn" type="button" data-smart-save ${valid ? "" : "disabled"}>${valid ? "Save Plan" : "Adjust Settings"}</button></div></div>`;
}
