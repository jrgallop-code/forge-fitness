...
const CORE_FULL_BODY = ["Chest", "Back", "Quads", "Hamstrings"];
const GOAL_LABELS = {
  muscle: "Build Muscle",
...
function isFullBodyGuardrailMode() {
  return shadow.days === 2 || shadow.days === 3;
}
...
function updateShadowFromButton(button) {
  ...
  if (button.dataset.removePreferred) {
    shadow.preferredIds = shadow.preferredIds.filter(item => item !== button.dataset.removePreferred);
  }

  if (button.dataset.removeExcluded) {
    shadow.excludedIds = shadow.excludedIds.filter(item => item !== button.dataset.removeExcluded);
  }
}
...
document.addEventListener("click", event => {
  ...
  if (button.matches("[data-smart-next]") && isGenerateStep() && !isFullBodyGuardrailMode()) generated = null;

  if (button.matches("[data-smart-next]") && isGenerateStep() && isFullBodyGuardrailMode()) {
    event.preventDefault();
    event.stopImmediatePropagation();
    generated = generateFullBodyProgram();
    renderGuardrailReview();
    return;
  }

  if (button.matches("[data-smart-regenerate-exercises]") && generated?.smartBuild?.version === 4) {
    event.preventDefault();
    event.stopImmediatePropagation();
    shadow.variation += 1;
    generated = generateFullBodyProgram();
  ...
  if (button.matches("[data-smart-regenerate-program]") && generated?.smartBuild?.version === 4) {
    event.preventDefault();
    event.stopImmediatePropagation();
    shadow.variation += 4;
    generated = generateFullBodyProgram();
  ...
  if (button.matches("[data-smart-save]") && generated?.smartBuild?.version === 4) {
    event.preventDefault();
    event.stopImmediatePropagation();
...
function generateFullBodyProgram() {
  ...
  days.forEach((day, dayIndex) => {
    CORE_FULL_BODY.forEach(muscle => {
      addExercise(days, dayIndex, muscle, 2, pool, usedCounts, { core: true, preferCompound: true });
    });
  });

  const floors = getCoverageFloors(weeklyCapacity);
  fillEffectiveDeficits(days, floors, pool, exerciseMap, usedCounts, perSessionCapacity, { hardFloor: true });

  const targets = getWeeklyTargets(floors);
  const priorityTargets = Object.fromEntries(
    Object.entries(targets).filter(([muscle]) => shadow.priorities.includes(muscle))
  );
  fillEffectiveDeficits(days, priorityTargets, pool, exerciseMap, usedCounts, perSessionCapacity, { priority: true });

  const normalTargets = Object.fromEntries(
    Object.entries(targets).filter(([muscle]) => !shadow.priorities.includes(muscle))
  );
  fillEffectiveDeficits(days, normalTargets, pool, exerciseMap, usedCounts, perSessionCapacity, {});
  ...
  const coverage = validateFullBodyCoverage(days, floors, effective, exerciseMap);
...
function getCoverageFloors(weeklyCapacity) {
  const constrained = weeklyCapacity < 28;
  if (shadow.goal === "strength" || shadow.goal === "maintain") {
    return {
      Chest: 4, Back: 4, Shoulders: constrained ? 2 : 3, Biceps: 2, Triceps: 2,
      Quads: 4, Hamstrings: 4, Glutes: 3, Calves: constrained ? 2 : 3
    };
  }

  return {
    Chest: constrained ? 4 : 6,
    Back: constrained ? 4 : 6,
    Shoulders: constrained ? 2 : 4,
    Biceps: constrained ? 2 : 4,
    Triceps: constrained ? 2 : 4,
    Quads: constrained ? 4 : 6,
    Hamstrings: constrained ? 4 : 5,
    Glutes: constrained ? 3 : 4,
    Calves: constrained ? 2 : 4
  };
}
...
function fillEffectiveDeficits(days, goals, pool, exerciseMap, usedCounts, perSessionCapacity, mode) {
  let guard = 400;
  while (guard-- > 0) {
    const effective = calculateEffectiveVolume(days, exerciseMap);
    ...
    for (const item of deficits) {
      if (addOneStimulusStep(days, item.muscle, pool, usedCounts, perSessionCapacity, mode)) {
        changed = true;
        break;
      }
    }
...
function addOneStimulusStep(days, muscle, pool, usedCounts, perSessionCapacity, mode) {
  const rankedDays = days
    .map((day, index) => ({
      index,
      totalSets: totalDaySets(day),
      muscleSets: primarySetsForMuscle(day, muscle)
    }))
  ...
  for (const slot of rankedDays) {
    ...
    if (room >= 2) {
      const added = addExercise(days, slot.index, muscle, 2, pool, usedCounts, {
        core: false,
        preferCompound: CORE_FULL_BODY.includes(muscle) && primarySetsForMuscle(day, muscle) === 0
      });
...
function pickExercise(day, muscle, pool, usedCounts, dayIndex, preferCompound) {
  ...
  return [...candidates].sort((a, b) => {
    return scoreExercise(b, usedCounts, dayIds, dayIndex, preferCompound)
      - scoreExercise(a, usedCounts, dayIds, dayIndex, preferCompound);
  })[0] || null;
}
...
function exerciseOrderScore(item, exerciseMap) {
  ...
  if (item._core) score += 25;
  if (exercise?.type === "compound") score += 12;
  if (item.primaryMuscle === "Chest") score += 4;
  if (item.primaryMuscle === "Back") score += 3;
  return score;
}
...
function applyAccessorySupersets(days, exerciseMap) {
  ...
  days.forEach(day => {
    const eligible = day.exercises.filter(item => !item._core && !isHighFatigueCompound(item, exerciseMap));
    let group = 0;
...
function validateFullBodyCoverage(days, floors, effective, exerciseMap) {
  const dayChecks = days.map(day => {
    const primary = new Set(day.exercises.map(item => normalizedDisplayMuscle(exerciseMap.get(item.id)?.muscleGroup || item.primaryMuscle)));
    return {
      name: day.name,
      push: primary.has("Chest"),
      pull: primary.has("Back"),
      knee: primary.has("Quads"),
      posterior: primary.has("Hamstrings") || primary.has("Glutes")
    };
  });
  ...
  const failedDays = dayChecks.filter(check => !(check.push && check.pull && check.knee && check.posterior));
  const failedMuscles = floorChecks.filter(check => !check.pass);

  return {
    passed: failedDays.length === 0 && failedMuscles.length === 0,
    dayChecks,
    floorChecks,
    failedDays,
    failedMuscles
...
function renderGuardrailReview() {
  ...
  if (heading) heading.textContent = `${GOAL_LABELS[shadow.goal]} Full Body Program`;
  ...
  const coverageStatus = coverage.passed
    ? `<div class="smart-volume-grid"><div class="priority"><span>Full-body coverage</span><strong>Passed ✓</strong></div><div><span>Every workout</span><strong>Push · Pull · Knee · Posterior</strong></div></div>`
    : `<div class="smart-volume-grid"><div><span>Full-body coverage</span><strong>Needs attention</strong></div></div><p class="smart-helper">${escapeHtml(formatCoverageFailure(coverage))}</p>`;
  ...
  host.innerHTML = `<div class="smart-review">
    ...
    <div class="smart-review-meta"><span>${shadow.days} full-body days/week</span><span>~${shadow.duration} min/session</span><span>${capitalize(shadow.experience)}</span><span>${GOAL_LABELS[shadow.goal]}</span></div>
    <h4>Full-body guardrails</h4>
    <p class="smart-helper">Priority changes emphasis, not coverage. Push, pull, knee-dominant and posterior-chain work are protected in every session before extra focus volume is added.</p>
    ${coverageStatus}
    <h4>Weekly muscle stimulus</h4>
    <p class="smart-helper">Floors are protected first. Priority volume is added only after those minimums are covered. Compound exercises contribute 0.5 effective-set credit to commonly involved secondary muscles.</p>
    <div class="smart-volume-grid effective">${effectiveRows}</div>
    <h4>Workout days</h4>
    <div class="smart-review-days">${days}</div>
    <p class="smart-review-note">These are starting guardrails rather than universal optimal-set claims. Very short schedules may use lower minimum effective-volume floors so every major region remains represented without creating unrealistic sessions.</p>
...
function formatCoverageFailure(coverage) {
  const parts = [];
  if (coverage.failedDays.length) parts.push(`${coverage.failedDays.map(item => item.name).join(", ")} is missing a required full-body movement pattern`);
...
function buildPlanName() {
  ...
  return `${shadow.days}-Day ${GOAL_LABELS[shadow.goal]} Full Body Program${priority}`;
}
