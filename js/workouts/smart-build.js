...
function handleClick(root,event){
  ...
  else if(button.matches("[data-picker-toggle]")){const p=root.querySelector("[data-picker-panel]");if(p){p.hidden=!p.hidden;if(!p.hidden)renderExerciseResults(root,root.querySelector("[data-smart-exercise-search]")?.value||"");}return;}
...
function handleInput(root,event){if(event.target.matches?.("[data-smart-exercise-search]"))renderExerciseResults(root,event.target.value);}
...
function renderEquipmentStep(){const options=["Full Gym","Barbell","Dumbbells","Machines & Cables","Bodyweight","Home Gym"];return questionCard("4","Choose your equipment","Select everything you have access to. Full Gym includes all equipment types.",`<strong class="smart-field-label">Available equipment</strong><div class="smart-chip-grid">${options.map(x=>`<button type="button" class="smart-chip ${state.equipment.includes(x)?"selected":""}" data-equipment="${x}">${x}</button>`).join("")}</div><p class="smart-helper">Selected: ${state.equipment.join(", ")}</p><div class="smart-picker-block"><button class="smart-picker-toggle" type="button" data-picker-toggle>Exercise preferences ▾</button><div class="smart-picker-panel" data-picker-panel hidden><input type="search" data-smart-exercise-search placeholder="Search exercise, muscle or equipment"><div class="smart-search-results" data-smart-search-results></div></div></div><div class="smart-pref-columns"><div><strong>Preferred</strong><div class="smart-selected-list">${renderSelectedExercises(state.preferredIds,"preferred")}</div></div><div><strong>Avoid / discomfort</strong><div class="smart-selected-list">${renderSelectedExercises(state.excludedIds,"excluded")}</div></div></div>`);}
...
function renderExerciseResults(root,q){const host=root.querySelector("[data-smart-search-results]");if(!host)return;const term=String(q||"").trim().toLowerCase();const results=getAllExercises().filter(e=>e.trackingType!=="notes").filter(e=>!term||[e.name,e.muscleGroup,e.equipment].some(v=>String(v||"").toLowerCase().includes(term))).sort((a,b)=>String(a.muscleGroup).localeCompare(String(b.muscleGroup))||String(a.name).localeCompare(String(b.name))).slice(0,40);host.innerHTML=results.length?results.map(e=>`<div class="smart-search-row"><div><strong>${escapeHtml(e.name)}</strong><small>${escapeHtml(e.muscleGroup)} · ${escapeHtml(e.equipment)}</small></div><button type="button" data-prefer-id="${escapeHtml(e.id)}">Prefer</button><button type="button" data-exclude-id="${escapeHtml(e.id)}">Avoid</button></div>`).join(""):`<p class="smart-helper">No matching exercises.</p>`;}
