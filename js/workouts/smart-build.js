...
function handleClick(root,event){
  ...
  if(button.matches("[data-smart-next]")){if(state.step===5){state.generated=generateProgram();state.step=6;}else state.step+=1;renderStep(root);return;}
...
function renderStep(root){const host=root.querySelector("[data-smart-step]"),progress=root.querySelector("[data-smart-progress]"),heading=root.querySelector("[data-smart-heading]");if(!host||!progress)return;const steps=[renderGoalStep,renderScheduleStep,renderExperienceStep,renderPriorityStep,renderEquipmentStep,renderProgrammingStep,renderResultStep];progress.style.width=`${((state.step+1)/steps.length)*100}%`;if(heading)heading.textContent=`${GOALS[state.goal].label} Program`;host.innerHTML=steps[state.step]();}
