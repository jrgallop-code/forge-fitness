...
function handleClick(root,event){
  ...
  else if(button.matches("[data-custom-equipment-toggle]")){const p=root.querySelector("[data-custom-equipment-panel]");if(p)p.hidden=!p.hidden;return;}
...
function renderEquipmentStep(){
  ...
  const custom=getEquipmentOptions().filter(x=>!presets.includes(x));
  return questionCard("4","Choose your equipment","Select everything you have access to. Full Gym includes all equipment types.",`<strong class="smart-field-label">Quick options</strong><div class="smart-chip-grid">${presets.map(x=>`<button type="button" class="smart-chip ${state.equipment.includes(x)?"selected":""}" data-equipment="${escapeHtml(x)}">${escapeHtml(x)}</button>`).join("")}</div><div class="smart-picker-block"><button class="smart-picker-toggle" type="button" data-custom-equipment-toggle>Choose your own equipment ▾</button><div class="smart-picker-panel" data-custom-equipment-panel hidden><div class="smart-chip-grid">${custom.map(x=>`<button type="button" class="smart-chip ${state.equipment.includes(x)?"selected":""}" data-equipment="${escapeHtml(x)}">${escapeHtml(x)}</button>`).join("")}</div></div></div><p class="smart-helper">Selected: ${state.equipment.join(", ")}</p><div class="smart-picker-block"><button class="smart-picker-toggle" type="button" data-preferred-toggle>Preferred exercises ▾</button><div class="smart-picker-panel" data-preferred-panel hidden><input type="search" data-preferred-search placeholder="Search exercises to prefer"><div class="smart-search-results" data-preferred-results></div></div></div><div class="smart-selected-list">${renderSelectedExercises(state.preferredIds,"preferred")}</div><div class="smart-picker-block"><button class="smart-picker-toggle" type="button" data-avoid-toggle>Avoid / discomfort ▾</button><div class="smart-picker-panel" data-avoid-panel hidden><input type="search" data-avoid-search placeholder="Search exercises to avoid"><div class="smart-search-results" data-avoid-results></div></div></div><div class="smart-selected-list">${renderSelectedExercises(state.excludedIds,"excluded")}</div>`);
}
...
function getEquipmentOptions(){return [...new Set(getAllExercises().map(e=>String(e.equipment||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));}
