...
function slideFrame(kind, eyebrow, title, content, profileName) {
  return `<article class="workout-complete-recap__slide is-${kind}" data-recap-slide="${kind}"><div class="workout-complete-recap__card-head"><span>${escapeHtml(eyebrow)}</span><b>${escapeHtml(title)}</b></div>${content}<footer><div class="workout-complete-recap__brand-lockup"><img src="${BRAND_MARK_URL}" alt=""><b><i>LEVEL</i><em>UP</em></b></div><span class="workout-complete-recap__profile-name">${profileName ? `@${escapeHtml(profileName.replace(/^@/, ""))}` : "LEVELUPHYPERTROPHY.COM"}</span></footer></article>`;
}
