...
function slideFrame(kind, eyebrow, title, content, profileName) {
  return `<article class="workout-complete-recap__slide is-${kind}" data-recap-slide="${kind}"><div class="workout-complete-recap__card-head"><span>${escapeHtml(eyebrow)}</span><b>${escapeHtml(title)}</b></div>${content}<footer><img src="${BRAND_MARK_URL}" alt="Level Up"><span>${profileName ? `@${escapeHtml(profileName.replace(/^@/, ""))}` : "LEVELUPHYPERTROPHY.COM"}</span></footer></article>`;
}
...
function initializeShareActions(overlay,payload) { overlay.querySelectorAll("[data-recap-share]").forEach(button=>button.addEventListener("click",async()=>{const action=button.dataset.recapShare,status=overlay.querySelector("[data-recap-share-status]");button.disabled=true;status.textContent="Preparing your workout card…";try{const index=Number(overlay.dataset.activeRecapSlide||0),blob=await createShareImage(payload,index),file=new File([blob],`level-up-${slug(payload.dayName)}.png`,{type:"image/png"});if(action==="download"){if(await saveImageToPhotos(blob)){status.textContent="Workout card saved to Photos.";}else{downloadBlob(blob,file.name);status.textContent="Workout card downloaded.";}return;}if(action==="instagram"&&await openInstagramStories(blob)){status.textContent="Opening Instagram Stories…";return;}if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){status.textContent=action==="instagram"?"Choose Instagram in the share sheet.":"Opening share options…";await navigator.share({title:`${payload.dayName} · Level Up`,text:shareText(payload),files:[file]});status.textContent="Workout shared.";}else{downloadBlob(blob,file.name);status.textContent="Image saved for sharing.";}}catch(error){status.textContent=error?.name==="AbortError"?"Sharing cancelled.":"Couldn't prepare that workout card. Please try again.";}finally{button.disabled=false;}})); }

async function openInstagramStories(blob) { const plugin=window.Capacitor?.Plugins?.LevelUpInstagramShare;if(!window.Capacitor?.isNativePlatform?.()||!plugin?.shareToStories)return false;const imageData=await blobToBase64(blob);const result=await plugin.shareToStories({imageData,contentUrl:APP_URL});return result?.opened===true; }
...
async function createShareImage(data,index) { const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1350;const ctx=canvas.getContext("2d"),palette=getSharePalette(data.shareTheme),{accent,bg,card,text,muted,line}=palette;ctx.fillStyle=bg;ctx.fillRect(0,0,1080,1350);const glow=ctx.createRadialGradient(540,430,20,540,430,620);glow.addColorStop(0,hexWithAlpha(accent,"32"));glow.addColorStop(1,"rgba(0,0,0,0)");ctx.fillStyle=glow;ctx.fillRect(0,0,1080,1100);roundRect(ctx,70,70,940,1160,58);ctx.fillStyle=card;ctx.fill();ctx.strokeStyle=hexWithAlpha(accent,"88");ctx.lineWidth=3;ctx.stroke();const titles=["YOU CRUSHED IT","TODAY'S WORK","THE WORK ADDS UP","LEVEL UP!","TOP PERFORMANCES"],brows=[`WORKOUT #${data.workoutNumber}`,"MUSCLES TRAINED","TRAINING TOTALS","SESSION WINS","STRONGEST SETS"];drawText(ctx,brows[index]||brows[0],540,150,24,800,accent);drawText(ctx,titles[index]||titles[0],540,218,53,950,text);if(index===1)await drawMuscleShareCard(ctx,data,palette);else if(index===2)drawTotalsShareCard(ctx,data,palette);else if(index===3)drawAchievementShareCard(ctx,data,palette);else if(index===4)drawTopSetsShareCard(ctx,data,palette);else drawCelebrationShareCard(ctx,data,palette);await drawBrandFooter(ctx,data,palette);drawText(ctx,"leveluphypertrophy.com",540,1300,23,700,accent);return new Promise((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(new Error("Image export failed")),"image/png",.95)); }
function drawCelebrationShareCard(ctx,data,p){drawText(ctx,"YOU ARE",540,350,35,900,p.text);drawText(ctx,"CRUSHING IT",540,465,94,950,p.accent);drawText(ctx,"TODAY!",540,530,42,900,p.text);drawStatPill(ctx,150,690,240,170,String(data.stats.workingSets),"WORKING SETS",p);drawStatPill(ctx,420,690,240,170,String(data.stats.totalReps),"TOTAL REPS",p);drawStatPill(ctx,690,690,240,170,formatDurationShort(data.session.durationMs),"DURATION",p);drawText(ctx,data.dayName,540,960,42,850,p.text);drawText(ctx,`${data.wins.length} ${data.wins.length===1?"WIN":"WINS"} THIS WORKOUT`,540,1015,24,800,p.accent);}
...
function drawAchievementShareCard(ctx,data,p){const prs=data.wins.filter(w=>/PR/.test(w.type)).length;drawText(ctx,"🏆",540,360,100,900,p.text);drawText(ctx,String(prs),540,515,130,950,p.accent);drawText(ctx,"PERSONAL RECORDS",540,570,30,850,p.text);data.wins.slice(0,3).forEach((win,i)=>{const y=680+i*125;drawText(ctx,win.type,245,y,22,800,p.accent,"left");drawFittedText(ctx,win.title,245,y+38,25,750,p.text,"left",390);drawFittedText(ctx,win.value,880,y+20,28,900,p.text,"right",230);});}
...
async function drawMuscleShareCard(ctx,data,p){try{const [front,back]=await Promise.all([makeAnatomyImage("front",data.trained,p.accent),makeAnatomyImage("back",data.trained,p.accent)]);ctx.drawImage(front,210,295,260,650);ctx.drawImage(back,610,295,260,650);}catch{drawText(ctx,data.trained.join(" · ")||"Workout complete",540,580,35,800,p.accent);}drawText(ctx,`${data.trained.length} MUSCLE GROUPS · ${data.stats.workingSets} WORKING SETS`,540,1010,28,850,p.text);}
...
async function drawBrandFooter(ctx,data,p){ctx.strokeStyle=p.line;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(120,1090);ctx.lineTo(960,1090);ctx.stroke();try{const logo=await loadImage(BRAND_MARK_URL);ctx.drawImage(logo,110,1100,145,120);}catch{drawText(ctx,"LEVEL UP",120,1160,27,900,p.text,"left");}drawFittedText(ctx,data.profileName?`@${data.profileName.replace(/^@/,"")}`:"LEVELUPHYPERTROPHY.COM",930,1160,24,700,p.muted,"right",570);}
