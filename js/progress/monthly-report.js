...
function ensureStyles() {
    ...
    link.id = STYLE_ID;
    link.rel = "stylesheet";
    link.href = "css/monthly-report.css?v=pwa-monthly-reports-1";
...
function renderReport(report) {
    ...
    return '<header class="monthly-report-view-head">' +
        ...
        '<button class="primary-btn" type="button" data-monthly-pdf>Save Full PDF</button><p data-monthly-export-status aria-live="polite"></p></footer>';
}
...
async function exportPdf(report, screen, button) {
    ...
    try {
        ...
        if (!printWindow) {
            downloadBlob(new Blob([html], { type: "text/html;charset=utf-8" }), "Level-Up-" + report.monthKey + "-Performance-Report.html");
            if (status) status.textContent = "Printable report downloaded. Open it and choose Print → Save as PDF.";
            return;
        }
        printWindow.document.open();
        printWindow.document.write(html);
        printWindow.document.close();
        printWindow.document.title = "Level Up " + report.label + " Performance Report";
        window.setTimeout(function () {
            try {
                printWindow.focus();
                printWindow.print();
            } catch (error) {
                console.error("Monthly report print dialog failed:", error);
            }
        }, 450);
        if (status) status.textContent = "Report opened. Choose Save as PDF in the print dialog.";
    } catch (error) {
        if (printWindow && !printWindow.closed) printWindow.close();
        console.error("Monthly report PDF export failed:", error);
        if (status) status.textContent = error && error.message ? error.message : "The report could not be created.";
    } finally {
...
function pdfWeightChartSvg(weight) {
    ...
    const pad = { left: 64, right: 28, top: 20, bottom: 48 };
    const minRaw = Math.min.apply(null, values), maxRaw = Math.max.apply(null, values);
    const span = Math.max(.5, maxRaw - minRaw);
    let minimum = minRaw - Math.max(.6, span * .12);
    let maximum = maxRaw + Math.max(.6, span * .12);
    if (Number.isFinite(weight.goalWeight)) {
    ...
    const x = function (date) { return pad.left + ((dateMs(date) - startMs) / elapsed) * plotWidth; };
    const y = function (value) { return pad.top + ((maximum - value) / Math.max(.1, maximum - minimum)) * plotHeight; };
    ...
    for (let index = 0; index <= 4; index += 1) {
        ...
        grid += '<line x1="' + pad.left + '" y1="' + yy.toFixed(1) + '" x2="' + (width - pad.right) + '" y2="' + yy.toFixed(1) + '"/>';
        yLabels += '<text class="axis-label" x="' + (pad.left - 8) + '" y="' + (yy + 3).toFixed(1) + '" text-anchor="end">' + value.toFixed(1) + '</text>';
    }

    const midMs = startMs + elapsed / 2;
    const midDate = localDate(new Date(midMs));
    const xLabels =
        '<text class="axis-label" x="' + pad.left + '" y="' + (height - 24) + '" text-anchor="start">' + escapeHtml(formatPdfAxisDate(weight.startDate)) + '</text>' +
        '<text class="axis-label" x="' + (pad.left + plotWidth / 2).toFixed(1) + '" y="' + (height - 24) + '" text-anchor="middle">' + escapeHtml(formatPdfAxisDate(midDate)) + '</text>' +
        '<text class="axis-label" x="' + (width - pad.right) + '" y="' + (height - 24) + '" text-anchor="end">' + escapeHtml(formatPdfAxisDate(weight.endDate)) + '</text>';
    ...
    const goal = Number.isFinite(weight.goalWeight)
        ? '<line class="target" x1="' + pad.left + '" y1="' + y(weight.goalWeight).toFixed(1) + '" x2="' + (width - pad.right) + '" y2="' + y(weight.goalWeight).toFixed(1) + '"/>' +
          '<text class="goal-label" x="' + (width - pad.right - 4) + '" y="' + Math.max(pad.top + 10, y(weight.goalWeight) - 5).toFixed(1) + '" text-anchor="end">Goal ' + weight.goalWeight.toFixed(1) + ' lb</text>'
        : "";

    return '<svg viewBox="0 0 ' + width + ' ' + height + '" role="img" aria-label="Monthly weight trend with goal weight">' +
        '<g class="grid">' + grid + '</g>' +
        yLabels +
        ...
        goal +
        xLabels +
        '<text class="axis-title" x="' + (pad.left + plotWidth / 2).toFixed(1) + '" y="' + (height - 4) + '" text-anchor="middle">Date</text>' +
        '<text class="axis-title" x="14" y="' + (pad.top + plotHeight / 2).toFixed(1) + '" text-anchor="middle" transform="rotate(-90 14 ' + (pad.top + plotHeight / 2).toFixed(1) + ')">Weight (lb)</text>' +
        '</svg>';
}

function formatPdfAxisDate(value) {
...
function pdfCss() {
    ...
    '.pdf-bars{display:grid;gap:8px;margin:12px 0 22px}.pdf-bar{display:grid;grid-template-columns:minmax(170px,1.2fr) minmax(150px,1.4fr) 64px;align-items:center;gap:10px;font-size:10px}.pdf-bar>span{min-width:0;white-space:normal;overflow-wrap:anywhere;line-height:1.25}.pdf-bar-track{height:10px;border-radius:999px;background:#e7e7e9;overflow:hidden}.pdf-bar-fill{height:100%;background:#e51b26}.pdf-bar strong{text-align:right;white-space:nowrap}' +
...
async function shareReportSummary(report, screen, button) {
    ...
    try {
        ...
        const filename = "Level-Up-" + report.monthKey + "-Summary.png";
        const file = new File([blob], filename, { type: "image/png" });
        if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
            await navigator.share({
                title: "Level Up " + report.label + " Performance Report",
                text: "My " + report.label + " Level Up performance summary.",
                files: [file]
            });
            if (status) status.textContent = "Summary shared.";
        } else {
            downloadBlob(blob, filename);
            if (status) status.textContent = "Summary downloaded.";
        }
    } catch (error) {
        if (error && error.name === "AbortError") {
            if (status) status.textContent = "Sharing cancelled.";
        } else {
