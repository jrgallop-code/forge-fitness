...
function isCompletedWorkingSet(set) {
    ...
    if (Object.prototype.hasOwnProperty.call(set, "completed")) return set.completed === true;
    return true;
}

function completedSetCount(exercise) {
    return Array.isArray(exercise?.sets)
        ? exercise.sets.filter(isCompletedWorkingSet).length
        : 0;
}
...
function setMode(shell, mode) {
    shell.dataset.muscleMode = mode;
    shell.querySelectorAll("[data-muscle-overview-mode]").forEach(button => {
        const active = button.dataset.muscleOverviewMode === mode;
        button.classList.toggle("active", active);
        button.setAttribute("aria-selected", active ? "true" : "false");
    });
    shell.querySelectorAll("[data-muscle-overview-panel]").forEach(panel => {
        panel.hidden = panel.dataset.muscleOverviewPanel !== mode;
    });

    if (mode === "volume") renderSevenDayVolume();
}

function buildOverview(trainingView, recoveryView) {
    const existingTrainingChildren = [...trainingView.children];
    const shell = document.createElement("div");
    shell.className = "muscle-overview-shell";
    shell.dataset.muscleOverviewReady = "true";
    shell.dataset.muscleMode = "volume";
    shell.innerHTML = `
        ...
        <section data-muscle-overview-panel="volume">
            ...
            <div data-muscle-volume-analytics></div>
        </section>
        <section class="muscle-overview-recovery-panel" data-muscle-overview-panel="recovery" hidden></section>
    `;

    const analyticsHost = shell.querySelector("[data-muscle-volume-analytics]");
    existingTrainingChildren.forEach(child => analyticsHost.appendChild(child));

    const recoveryPanel = shell.querySelector('[data-muscle-overview-panel="recovery"]');
    recoveryView.classList.remove("training-progress-view");
    recoveryView.removeAttribute("data-view");
    recoveryView.hidden = false;
    recoveryPanel.appendChild(recoveryView);

    trainingView.appendChild(shell);

    shell.querySelectorAll("[data-muscle-overview-mode]").forEach(button => {
        button.addEventListener("click", () => setMode(shell, button.dataset.muscleOverviewMode));
    });
...
function enhanceMuscleOverview() {
    const trainingView = document.querySelector('.training-progress-view[data-view="training"]');
    const trainingTab = document.querySelector('.training-progress-tab[data-view="training"]');
    const recoveryView = document.querySelector('.training-progress-view[data-view="recovery"]');
    const recoveryTab = document.querySelector('.training-progress-tab[data-view="recovery"]');
    if (!trainingView || !trainingTab) return;
    ...
    recoveryTab?.remove();
    buildOverview(trainingView, recoveryView);
}

function queueEnhancement() {
    requestAnimationFrame(enhanceMuscleOverview);
    window.setTimeout(enhanceMuscleOverview, 60);
}

document.addEventListener("click", event => {
    if (event.target.closest?.('#lifting-tab, .training-progress-tab[data-view="training"], #load-training-demo, #remove-training-demo')) {
        window.setTimeout(() => {
