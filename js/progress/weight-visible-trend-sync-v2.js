...
const STYLE_ID = "level-up-weight-trend-interaction-fix-v2";
let queued = false;
...
function syncTopSummary(weights) {
    ...
    if (trendWeightNode) setText(trendWeightNode, Number.isFinite(trendWeight) ? `${trendWeight.toFixed(1)} lb` : "--");
    ...
    if (rateNode) {
        ...
        setText(rateNode.closest(".metric-card")?.querySelector("h3"), trend.label || "Weekly Trend");
    }
}
