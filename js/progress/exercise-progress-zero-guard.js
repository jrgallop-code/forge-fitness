...
export function initializeExerciseProgressZeroGuard() {
    ...
    const refresh = () => {
        ...
        attachOnce(document.getElementById("progress-range"), "change", () => requestAnimationFrame(renderGuardedExerciseProgress));
        attachOnce(document.getElementById("lifting-tab"), "click", () => requestAnimationFrame(renderGuardedExerciseProgress));
...
function renderGuardedExerciseProgress() {
    ...
    drawPositiveOnlyLineChart(canvas, records.map(record => ({ label: formatShortDate(record.date), value: record.estimatedOneRepMax })));
    if (!records.length) {
        history.innerHTML = `<p class="empty-state">No completed sets with reps above 0 for this exercise yet.</p>`;
        return;
    }
    history.innerHTML = [...records].reverse().map(record => `
...
function getValidExerciseRecords(sessions, exerciseId) {
    ...
    return sessions.map(session => {
        const matchingExercises = Array.isArray(session.exercises) ? session.exercises.filter(item => item?.exerciseId === exerciseId) : [];
        const validSets = matchingExercises.flatMap(exercise => Array.isArray(exercise.sets) ? exercise.sets : []).filter(isPerformedSet);
        if (!validSets.length) return null;
        const scoredSets = validSets.map(set => ({ set, estimate: estimateOneRepMax(set) })).filter(item => Number.isFinite(item.estimate) && item.estimate > 0).sort((a, b) => b.estimate - a.estimate);
        if (!scoredSets.length) return null;
        return { date: session.date, bestSet: scoredSets[0].set, estimatedOneRepMax: scoredSets[0].estimate, completedSets: validSets.length };
    }).filter(record => record && Number.isFinite(record.estimatedOneRepMax) && record.estimatedOneRepMax > 0);
}
...
function getFilteredSessions() {
    ...
    if (!stored) return [];
    let sessions;
    try { sessions = JSON.parse(stored); } catch { return []; }
    ...
    if (days > 0) {
        ...
        filtered = sessions.filter(session => new Date(`${session.date}T23:59:59`) >= cutoff);
    }
...
function drawPositiveOnlyLineChart(canvas, incomingPoints) {
    const points = incomingPoints.filter(point => Number.isFinite(Number(point.value)) && Number(point.value) > 0);
    ...
    for (let tick = 0; tick <= 4; tick++) {
        ...
        const y = height - padding.bottom - (value - axisMinimum) / axisRange * (height - padding.top - padding.bottom);
    ...
    const coordinates = points.map((point, index) => ({
        ...point,
        x: padding.left + (points.length === 1 ? (width - padding.left - padding.right) / 2 : index / (points.length - 1) * (width - padding.left - padding.right)),
        y: height - padding.bottom - (Number(point.value) - axisMinimum) / axisRange * (height - padding.top - padding.bottom)
    }));
    ...
    coordinates.forEach((point, index) => {
        ...
        context.fill();
        if (shouldShowXAxisLabel(index, coordinates.length)) {
            context.fillStyle = "#a0a0a0";
            context.font = "11px Arial";
            context.textAlign = "center";
...
function formatSet(set) { return `${Number(set.weight)} × ${Number(set.reps)}`; }
