...
const STEPS = [
    { target: "#begin-session-btn", title: "Start logging the workout", body: "Normally, go to Workout → My Workouts, choose your plan, and tap Log a Workout. Confirm the Training Day and Workout Date here, then tap Begin Workout. This starts the workout timer and opens the exercise-by-exercise logger." },
    { target: `${PRIMARY_SELECTOR} .logger-form-guide-btn`, title: "Use the Form Guide", body: "Tap Form Guide whenever you want a technique refresher before or during an exercise. It opens the exercise guide with setup cues and the form video, then returns you to the same place in the workout when you are done." },
    { target: `${PRIMARY_SELECTOR} .exercise-warmup-btn`, title: "Show your warm-up sets", body: "Tap Warm-up to reveal the suggested warm-up sets for this lift. Warm-ups prepare you for the working weight and stay separate from the programmed working sets, so they do not replace Set 1, Set 2, or Set 3." },
    { target: `${PRIMARY_SELECTOR} .complete-warmup-btn`, title: "Complete a warm-up", body: "After performing the warm-up, tap the completion button on that row. Level Up marks it finished and can start the normal rest-timer flow so you know when to move to the next set." },
    { target: `${PRIMARY_SELECTOR} .plate-calculator-trigger`, title: "Use the plate calculator", body: "For barbell exercises, the plate calculator reads the weight entered in the active set and shows exactly what to load on each side of the bar. Use it whenever the target load changes and you want to avoid plate math." },
    { target: `${PRIMARY_SELECTOR} .session-set-row[data-set-index="0"] .complete-set-btn`, title: "Log a working set", body: "Each working-set row shows previous performance, today’s Weight, today’s Reps, and the completion button. Enter what you actually lifted after the set, then tap Complete Set. Completed sets feed workout history, volume, strength trends, and progression guidance." },
    { target: `${PRIMARY_SELECTOR} .session-set-row[data-set-index="0"] .drop-set-menu-trigger`, title: "Add a drop set", body: "Tap the set-number action when you want to continue immediately at a lighter load after a working set. Level Up adds a Drop Set row and suggests a practical lighter weight. Enter the weight and reps you perform, then mark the drop complete." },
    { target: `${PRIMARY_SELECTOR} .session-inline-swap`, title: "Swap an exercise for today", body: "Tap Swap if equipment is busy or you need a different movement today. Smart Swap ranks alternatives with a similar training purpose. The change is for the active workout and does not rewrite the saved plan." },
    { target: `${FIRST_CARD_SELECTOR} .compact-add-set-btn`, title: "Add another working set", body: "Tap Add Set when you intentionally need one extra working set for this exercise. The new row follows the same Weight, Reps, and Complete workflow as the programmed sets." },
    { target: `${FIRST_CARD_SELECTOR} .session-add-exercise-btn`, title: "Add an exercise during the workout", body: "Tap Add Exercise to open the exercise library without leaving the session. Search by exercise name or browse by muscle, then choose the movement you want to add for today." },
    { target: "#save-session-btn", title: "Complete the workout", body: "When the session is finished, tap Complete Workout. In a normal workout this saves the session to Workout History and sends the completed set data into Progress, including volume and strength tracking." }
];
...
function removeLegacyTutorialArtifacts() {
    document.querySelectorAll(".interactive-workout-tutorial-section").forEach(section => {
        const card = section.querySelector(`#${CARD_ID}`);
        if (!card || card.dataset.tutorialVersion !== CARD_VERSION) section.remove();
    });
    [
        "level-up-interactive-workout-tutorial-styles",
        "level-up-interactive-workout-tutorial-v2-styles"
...
function injectTutorialCard() {
    ...
    removeLegacyTutorialArtifacts();
    if (document.querySelector(`#${CARD_ID}[data-tutorial-version="${CARD_VERSION}"]`)) return;
    ...
    card?.addEventListener("pointerdown", launch, { capture: true });
    card?.addEventListener("click", launch, { capture: true });
...
function startTutorial() {
    if (active || launchInProgress || !TUTORIAL_PLAN) return;
    launchInProgress = true;
    active = true;
    stepIndex = 0;
    flow = null;

    const card = document.querySelector(`#${CARD_ID}[data-tutorial-version="${CARD_VERSION}"]`);
    const state = card?.querySelector(".interactive-workout-tutorial-state");
    if (state) state.textContent = "Opening…";
...
function exitTutorial(completed) {
    ...
    waitFor('[data-more-page="learn"]', button => {
        button.click();
        if (completed) window.setTimeout(() => document.getElementById(CARD_ID)?.scrollIntoView({ behavior: "smooth", block: "center" }), 160);
    });
}
