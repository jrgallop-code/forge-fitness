...
export function initializeGoalsCalculationModeUI() {
    ...
    if (!goalsView || !manualPanel) return;

    const goalCards = Array.from(goalsView.querySelectorAll(":scope > .goal-box.nutrition-goal-card"));
    const autoPanel = goalCards.find(card => card !== manualPanel);
    if (!autoPanel) return;

    if (!document.getElementById("goal-calculation-mode-picker")) {
        autoPanel.insertAdjacentHTML("beforebegin", `
            <div id="goal-calculation-mode-picker" class="goal-calculation-mode-picker">
                <button id="goal-mode-auto" class="goal-calculation-mode-btn" type="button" data-goal-calculation-mode="auto">
                    <strong>Auto Calculate</strong><small>Use your profile, estimated TDEE and selected goal</small>
                </button>
                <button id="goal-mode-manual" class="goal-calculation-mode-btn" type="button" data-goal-calculation-mode="manual">
                    <strong>Manual Calculate</strong><small>Use your known maintenance and custom weekly target</small>
                </button>
            </div>
        `);
    }
    ...
    if (!autoPanel.querySelector(".goal-mode-panel-heading")) {
        autoPanel.insertAdjacentHTML("afterbegin", `<div class="goal-mode-panel-heading"><span class="eyebrow">AUTO CALCULATE</span><p>Choose a goal and Level Up will calculate a starting calorie target automatically.</p></div>`);
    }
    ...
    document.querySelectorAll("[data-active-target-source]").forEach(button => {
        button.addEventListener("click", () => {
            ...
            if (!target) {
                updateActiveTargetMessage(source === "manual" ? "Complete the manual calculation first." : "Save your Body Profile and nutrition goal first.");
                return;
            }
...
function ensureActiveTargetPicker(manualPanel) {
    ...
    manualPanel.insertAdjacentHTML("afterend", `
        <div id="active-target-picker" class="goal-box nutrition-goal-card active-target-picker">
            <span class="eyebrow">ACTIVE TARGET</span><h3>Dashboard Target</h3>
            ...
            <div class="active-target-actions">
                <button class="goal-calculation-mode-btn" type="button" data-active-target-source="auto"><strong>Use Auto Target</strong><small id="active-auto-target-value">--</small></button>
                <button class="goal-calculation-mode-btn" type="button" data-active-target-source="manual"><strong>Use Manual Target</strong><small id="active-manual-target-value">--</small></button>
            </div>
...
function renderActiveTargetPicker() {
    ...
    updateActiveTargetMessage(selected
        ? `Currently using ${selected.source === "manual" ? "Manual" : "Auto"} Target · ${selected.calories} kcal/day`
...
function updateActiveTargetMessage(message) { setText("active-target-message", message); }
function getCalculationMode() {
    const stored = localStorage.getItem(CALCULATION_MODE_KEY);
    if (stored === "auto" || stored === "manual") return stored;
    return localStorage.getItem(MANUAL_MAINTENANCE_KEY) !== null || localStorage.getItem(CUSTOM_WEEKLY_RATE_KEY) !== null ? "manual" : "auto";
}
...
function applyStoredMode() { if (getCalculationMode() === "auto") stashManualValues(); else restoreManualValues(); }
function stashManualValues() { stashValue(MANUAL_MAINTENANCE_KEY, SAVED_MANUAL_MAINTENANCE_KEY); stashValue(CUSTOM_WEEKLY_RATE_KEY, SAVED_CUSTOM_WEEKLY_RATE_KEY); }
function restoreManualValues() { restoreValue(SAVED_MANUAL_MAINTENANCE_KEY, MANUAL_MAINTENANCE_KEY); restoreValue(SAVED_CUSTOM_WEEKLY_RATE_KEY, CUSTOM_WEEKLY_RATE_KEY); }
function stashValue(activeKey, savedKey) { const value = localStorage.getItem(activeKey); if (value === null || value === "") return; localStorage.setItem(savedKey, value); localStorage.removeItem(activeKey); }
function restoreValue(savedKey, activeKey) { const value = localStorage.getItem(savedKey); if (value !== null && value !== "") localStorage.setItem(activeKey, value); }
