...
function renderAnalytics(data) {
    ...
    const peopleRows = (data.people || []).map(person => `<div class="admin-analytics-person"><div class="admin-analytics-person-identity"><strong>${escapeHtml(personName(person))}</strong><small>${escapeHtml(person.email || "")}</small><span>${escapeHtml(lastSeen(person.last_active_at))}</span></div><div class="admin-analytics-person-usage"><span><b>${number(person.active_days)}</b> active days</span><span class="is-foods"><b>${number(person.foods_logged)}</b> foods</span><span class="is-workouts"><b>${number(person.workouts_logged)}</b> workouts</span></div></div>`).join("");
...
function ensureUsageStyles() {
    ...
    link.rel = "stylesheet";
    link.href = "css/admin-analytics-usage.css?v=usage-people-1";
