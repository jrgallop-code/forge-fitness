...
const inlineSources = {};
for (const [key, sourceName] of Object.entries({
  "male-front":"front-view.svg", "male-back":"back-view.svg",
  "female-front":"female-front-view.svg", "female-back":"female-back-view.svg"
})) {
  const source = fs.readFileSync(path.join(ROOT, "assets/recovery", sourceName), "utf8");
  inlineSources[key] = source.replace(/^.*?<svg\b[^>]*>/s, "").replace(/<\/svg>\s*$/s, "");
}
const inlineModule = `// Generated by tools/generate-exercise-anatomy-assets.mjs.\nconst SOURCES = ${JSON.stringify(inlineSources)};\nexport function getInlineAnatomyBody(sex, side) { return SOURCES[sex + "-" + side] || ""; }\n`;
fs.writeFileSync(path.join(ROOT, "js/workouts/exercise-anatomy-inline.js"), inlineModule);
