...
async function createWorkoutShare(body, request, env) {
    const normalized = normalizeWorkoutSharePackage(body && body.workout ? body.workout : body);
    ...
    for (let attempt = 0; attempt < 8 && !inserted; attempt += 1) {
        ...
        try {
            await env.DB.prepare(
                "INSERT INTO workout_shares (code, payload_json, creator_user_id, created_at, expires_at, open_count) VALUES (?, ?, ?, ?, ?, 0)"
            ).bind(code, normalized.payloadJson, creator && creator.id ? creator.id : null, createdAt, expiresAt).run();
            inserted = true;
        }
        catch (error) {
...
async function getWorkoutShare(rawCode, request, env) {
    ...
    const row = await env.DB.prepare(
        "SELECT payload_json, expires_at FROM workout_shares WHERE code = ? AND expires_at > ?"
    ).bind(code, now).first();
...
function workoutShareLanding(rawCode) {
    ...
    const html =
        ...
        '<meta name="theme-color" content="#09090b"><title>Open Level Up Workout</title>' +
...
async function createPasswordCredential(user, body, request, env) {
    ...
    if (existing) {
        return json({ error: "A Level Up password is already configured for this account." }, 409, request, env);
    }
    ...
    try {
        ...
        console.error(JSON.stringify({ event: "password_link_failed", userId: user.id, message: String(error?.message || error) }));
...
async function getPlatformAnalytics(env, since, activeSince, today) {
    ...
    const [activity, engagement, versions, firstIos, signups, conversions, bothPlatforms] = await Promise.all([
        env.DB.prepare(`SELECT ${platformExpression} AS platform,
            ...
            FROM usage_events WHERE event_name = 'app_active' GROUP BY platform`).bind(since, activeSince, today.start, today.end).all(),
        env.DB.prepare(`SELECT platform,
            ...
            FROM (
                SELECT user_id, 'food' AS kind, ${platformExpression} AS platform FROM usage_events WHERE event_name = 'food_logged' AND occurred_at >= ?
                UNION ALL
                SELECT user_id, 'workout' AS kind, ${platformExpression} AS platform FROM product_events WHERE event_name = 'workout_completed' AND occurred_at >= ?
            ) GROUP BY platform`).bind(since, since).all(),
        env.DB.prepare(`SELECT COALESCE(NULLIF(json_extract(metadata_json, '$.appVersion'), ''), 'Unknown') AS app_version,
                COALESCE(NULLIF(json_extract(metadata_json, '$.appBuild'), ''), 'Unknown') AS app_build,
                COUNT(DISTINCT user_id) AS users, MAX(occurred_at) AS last_seen_at
            FROM usage_events WHERE event_name = 'app_active' AND occurred_at >= ? AND json_extract(metadata_json, '$.platform') = 'ios'
  ...
  <div style="max-width:640px;margin:0 auto;padding:28px 18px 36px;">
    ...
    <div style="border:1px solid #27272a;border-radius:18px;overflow:hidden;background:#111113;">
      <div style="padding:30px 26px 26px;border-top:4px solid #dc2626;">
        <img src="https://leveluphypertrophy.com/assets/level-up-email-logo.png" width="72" height="72" alt="Level Up" style="display:block;width:72px;height:72px;margin:0 0 14px;border:0;outline:none;text-decoration:none;" />
