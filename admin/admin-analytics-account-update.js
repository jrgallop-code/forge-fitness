...
function renderEmailDelivery(config) {
    ...
    return `<section class="admin-analytics-card admin-email-delivery">
        ...
        <div class="admin-email-draft">
            ...
            <div class="admin-email-live-send">
                <div>
            ...
            <div class="admin-email-actions">
                ...
                <p data-admin-ios-launch-status>${config?.iosLaunchSend?.status === "sent" ? `Sent once to ${number(config.iosLaunchSend.recipient_count)} registered account${Number(config.iosLaunchSend.recipient_count) === 1 ? "" : "s"} · ${escapeHtml(formatAnalyticsTime(config.iosLaunchSend.sent_at))}.` : "Not sent yet. Owner test clicks are not counted."}</p>
            </div>
        </div>
    </section>`;
}
