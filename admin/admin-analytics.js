...
function renderPlatformAnalytics(data, days) {
    ...
    return `<section class="admin-platform-section"><div class="admin-analytics-card-head"><div><span class="eyebrow">PLATFORMS</span><h3>iOS app and PWA</h3><p>Signed-in users who allowed analytics. Older activity remains Legacy/Unknown.</p></div></div><div class="admin-platform-grid">${card("ios", "iOS App", `${number(data.iosFirstSeenUsers)} first iOS activities`)}${card("pwa", "PWA", "Web and installed PWA")}${card("unknown", "Legacy / Unknown", "Recorded before platform tracking")}</div><div class="admin-platform-versions"><h4>iOS versions</h4>${versions || `<p class="admin-analytics-empty">Version usage will appear when the updated iOS app is used.</p>`}</div></section>`;
}
