...
function renderResults(target, data) {
  ...
  const rows = users.map(user => {
    ...
    return `<div class="admin-daily-user-row"><div><strong>${escapeHtml(personName(user))}</strong><small>${escapeHtml(user.email || "")}</small><em>Joined ${escapeHtml(formatTime(user.created_at))}</em></div><div>${activity.join("")}</div></div>`;
  }).join("");
  target.innerHTML = `<div class="admin-daily-user-summary"><div><span>Active users</span><strong>${number(data.activeUsers)}</strong></div><div><span>New accounts</span><strong>${number(data.newUsers)}</strong></div></div>
