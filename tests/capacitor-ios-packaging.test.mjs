...
test("native iOS packages adaptive monthly reports and classic PDF export", async () => {
    ...
    assert.doesNotMatch(report, /\.slice\(0, 5\)/);
    assert.match(report, /monthly-strength-swipe/);
    assert.match(report, /Swipe for more lifts/);
    assert.match(report, /Object\.keys\(getAnatomyConfig\("front"\)\.regions/);
    assert.match(report, /Object\.keys\(getAnatomyConfig\("back"\)\.regions/);
    assert.match(report, /row\.sets > 0/);
    assert.match(report, /buildPdfAnatomyData/);
    assert.match(report, /level-up-monthly-volume-overlay/);
    assert.match(report, /Goal /);
    assert.match(report, /Weight \(lb\)/);
    assert.match(report, />Date</);
    assert.match(report, /Strength · Continued/);
    assert.match(styles, /monthly-strength-swipe/);
    assert.match(styles, /overflow-wrap:anywhere/);
    assert.doesNotMatch(styles, /text-overflow:ellipsis;white-space:nowrap/);
