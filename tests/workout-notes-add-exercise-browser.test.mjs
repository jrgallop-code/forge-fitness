...
test('Add Exercise is inserted directly after Add Set and remains distinct from Smart Swap', () => {
  ...
  assert.match(actions, /session-swap-browse[^>]*>Choose Your Own</);
  assert.match(actions, /openAddExerciseSheet\(logger, \{ mode: 'swap', exerciseIndex \}\)/);
  assert.match(actions, /sheet\.dataset\.selectionMode === 'swap'/);
  assert.doesNotMatch(actions, /class="session-swap-select"/);
});
...
test('shared visual browser defaults to All and combines muscle with search', () => {
  ...
  assert.match(index, /session-exercise-actions\.js\?v=exercise-picker-swap-pwa-1/);
  assert.match(index, /workout-logger-compact\.js\?v=history-rir-edit-1/);
  assert.match(index, /session-exercise-actions\.css\?v=exercise-picker-swap-pwa-1/);
