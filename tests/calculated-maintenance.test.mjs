...
test("infers maintenance from intake and a losing smoothed weight trend", () => {
    ...
    assert.equal(result.maintenanceCalories, 2463);
    assert.equal(Math.round(result.energyCorrection), 163);
});
...
test("subtracts a gaining smoothed trend from average intake", () => {
    ...
    assert.equal(result.maintenanceCalories, 2570);
});
...
test("keeps calculated TDEE at whole-calorie precision instead of 25-calorie steps", () => {
    ...
    assert.equal(result.maintenanceCalories, 2583);
...
test("shows a usable early estimate from two food days and an established weight evidence span", () => {
    ...
    assert.equal(result.maintenanceCalories, 2398);
});
...
test("uses the smoothed rate for the energy correction", () => {
    ...
    assert.equal(Math.round(result.energyCorrection), 57);
    assert.equal(result.maintenanceCalories, 2678);
});
...
test("the validated real weigh-in pattern reflects the slower 10/90 trend response", () => {
    const weights = [
    ...
    assert.equal(result.weeklyChange.toFixed(2), "-0.10");
});
