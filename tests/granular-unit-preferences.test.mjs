...
test("workout surfaces use canonical lifting weights and unit-aware display", async () => {
  ...
  assert.match(plates, /displayMass\(value, 2, UNIT_KINDS\.LIFTING_WEIGHT\)/);
  ...
  assert.match(plates, /<div class="plate-calculator-settings" hidden>/);
  ...
  assert.match(plates, /data-plate-add=/);
  assert.match(plates, /startingLoad \+ plate \* 2/);
});
